...
public final class URLFormDecoder: Decoder {
    ...
    case let .formatted(formatter):
      guard let date = formatter.date(from: string) else {
        throw Error.decodingError(
          "Expected \(String(describing: formatter.dateFormat)), got \(string)",
          self.codingPath
        )
      }
      return date
  ...
  struct UnkeyedContainer: UnkeyedDecodingContainer {
    ...
    private mutating func checked<T>(_ block: (String) throws -> T) throws -> T {
    ...
    private mutating func unwrap<T>(_ block: (String) -> T?) throws -> T {
...
private let iso8601DateFormatter = DateFormatter(iso8601Format: "yyyy-MM-dd'T'HH:mm:ss.SSSXXXXX")

private let iso8601DateFormatterWithoutMilliseconds = DateFormatter(iso8601Format: "yyyy-MM-dd'T'HH:mm:ssXXXXX")
